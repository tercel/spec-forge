...
**Fast path (preferred):** resolve `<sf_scripts>` (see `@../shared/scripts.md`) and run `python3 "<sf_scripts>/sf-scan.py" --root "<project_root>"`. Its JSON supplies the tech stack, `frameworks`, DB/auth `signals`, `test.framework`/`test.command`, `source_tree`, and the existing-doc inventory in one pass — use those directly. Fall back to the manual scan below (silently) only when `python3` is unavailable.
...
The Test Plan must trace every test case back to a requirement. The `sf-scan.py` inventory from Step 1 already lists upstream docs under `document_index` (srs/tech-design/prd) with the requirement IDs each declares (`declared_ids`); use it instead of re-globbing.

1. **Locate upstream docs** from the scan's `document_index` (or, as a fallback, glob `docs/` for `*/srs.md`, `*/tech-design.md`, `*/prd.md`).
2. **Extract requirement IDs** (e.g., `FR-XXX-NNN`, `NFR-XXX-NNN`): use `python3 "<sf_scripts>/sf-trace.py" ids "<srs.md>"` (or the scan's `declared_ids`) rather than reading them out by hand — these seed the traceability matrix in Step 5.
...
After scanning, infer everything you can from the codebase, the upstream SRS/Tech Design, and any existing test plan — and state those inferences as assumptions the user can correct. Then ask **only** the questions whose answers materially change the plan and cannot be inferred. Typical areas that genuinely need the user (they are rarely in the code) include:
...
Proceed to generation once you have enough to fill the core sections — on your inferred defaults (stated explicitly) where the user has not weighed in — rather than blocking on a full questionnaire. Capture any remaining unknowns as open questions in the plan.
...
After generating the Test Plan, construct the Requirements Traceability Matrix (RTM). Map every SRS requirement ID (both functional and non-functional) to one or more test case IDs. Let the script layer do the coverage math: run

```bash
python3 "<sf_scripts>/sf-trace.py" matrix --upstream "<srs.md>" --downstream "<test-plan.md>"
```

and use its `uncovered_upstream` (requirements with no test case — the gaps to flag) and `orphan_downstream_refs` (test references to requirement IDs that do not exist). You decide whether each gap is intentional; the script does the bipartite counting. Fall back to building the matrix by hand if the script is unavailable. The RTM is the primary mechanism for proving that testing is complete and aligned with the specification.

### Step 6 -- Quality Check

Run the structural gate first — `python3 "<sf_scripts>/sf-verify-doc.py" "<test-plan.md>" --strict` deterministically checks the title, sections, TC ID format/uniqueness, and leftover placeholders — and fix everything it flags. Then validate the judgement items against `references/checklist.md` (the parts the script cannot judge). Summarize the checklist results so the user can see what passed and whether any items were intentionally skipped (with justification). (Run the checklist by hand if `python3` is unavailable.)
...
Test cases use the **same `TC-<MODULE>-<NNN>` convention as the `test-cases-generation` skill** — that skill is the single source of truth for the shared test-case conventions (ID format and the implementation-ready field structure); keep the two identical rather than letting them drift. In brief:
...
Examples: `TC-AUTH-001`, `TC-PAY-012`, `TC-CART-003`. Allocate the next free number with `python3 "<sf_scripts>/sf-trace.py" next-id --prefix TC --module AUTH --existing "<test-plan.md>"` to avoid collisions when extending.
...
Every test plan must include a dedicated section for data integrity test cases. Per the real-database rule above, these must run against a real DB (a mock cannot catch a constraint violation). Data integrity tests must cover:
