...
- **§3.6 Acceptance Criteria**: Write 5-15 testable AC entries mapping to the user scenarios. Tag each AC with a priority (P0/P1/P2) and specify the exact API call, test type, and assertion. The set must cover — for every P0 behavior — not only the happy path but its boundary, error, and negative conditions; a happy-path-only AC set is rejected. These ACs drive the Acceptance Criteria table in each feature spec (which is the standard `code-forge:review`'s Acceptance Gate reconciles against when no SRS/test-cases exist), so phrase each as a single mechanically-testable condition with a concrete expected result. If an idea draft exists (idea-first mode), extract the MVP requirements and success criteria from it as inputs for the AC table.
...
- **Error Handling**: Define error taxonomy with HTTP status, error codes, retry strategy, circuit breaker config, and fallback behavior for every external dependency. Enforce the §7.6 Core Principles: every anticipated failure (precondition, business rule, uniqueness/FK/check constraint, state guard, validation) returns a specific 4xx code — `500` is reserved exclusively for unanticipated faults. Fill the §7.6 Database Constraint → Error Translation table for every externally-reachable constraint in §10.
- **Error Catalog (§7.7)**: Build the single-source-of-truth error catalog. Every SRS alternative/error flow (AF-*), every §7.5 business-rule guard, every §7.3 validation rule, and every §7.6 constraint must appear as a catalog row mapping trigger → error code → HTTP status → endpoint(s) → message → test case. No anticipated failure may be left without a specific, non-500 entry. In §9.1, enumerate the Possible Error Codes for every endpoint, and never list 500 as a contract response.
...
- Do NOT leave Acceptance Criteria empty, write "TBD", or list only happy-path criteria. Map relevant ACs from §3.6 and add component-specific ones. Every row MUST carry a Priority (P0/P1/P2), a concrete Expected Result (the exact observable outcome — never "succeeds"/"fails"), and a specific Verification Method. For every P0 behavior, include not just the happy path but its boundary, error, and negative conditions — a happy-path-only AC set is rejected (it is the primary cause of demo-not-product). This table is the standard `code-forge:review`'s Acceptance Gate reconciles against when no SRS/test-cases exist, so each row must be mechanically mappable to exactly one test.
...
2. **Do NOT use "handle appropriately" or "validate as needed".** Every error condition must specify the exact HTTP status code, error code, retry behavior, and user-facing message. Every validation rule must specify the exact type, range, pattern, and error response.
   - **Do NOT let an anticipated failure fall through to 500.** Creating an already-existing record, referencing a missing foreign key, violating a state guard, or hitting any documented constraint are *known* failures and must return a specific 4xx code with a diagnosable message. A generic "500 Internal Server Error" for any failure described in this document is an automatic quality failure.
   - **Do NOT leak internals in error messages.** No stack traces, SQL fragments, raw driver exceptions (`UniqueViolation`, `IntegrityError`), or PII may appear in any user-facing message. Translate lower-layer exceptions into the taxonomy before they reach the client.
